...
class Extractor:
    ...
    def run(self):
        ...
        self.driver.press_keys(
            "data:element:search-form-input", self.variables["phrase"]
        )
    ...
    def check_topic(self, topics_list: list[str]):
        ...
        for topic in topics_list:
            ...
            topics_element = self.driver.find_elements(
                "tag:label", parent=topics_element
            )
            for index, t in enumerate(topics_element):
                if t.text.lower() == topic.lower():
                    self.driver.execute_javascript(
                        "document.getElementsByClassName"
                        f'("checkbox-input-label")[{index}].click()'
                    )
                    self.driver.wait_until_element_is_visible(
                        "class:search-results-module-filters-selected", 10
                    )
                    time.sleep(1)
                    self.driver.wait_until_element_is_visible(
                        "tag:ps-promo", 10
    ...
    def search_news(self):
        ...
        while stop_search:
            ...
            for e in elements:
                timestamp = get_element_attribute(
                    self.driver, "class:promo-timestamp", "data-timestamp", e
                )
    ...
    def get_news(self, element: WebElement):
        ...
        if element_exist(self.driver, "class:promo-description", element):
            description = self.driver.find_element(
                "class:promo-description", element
            ).text
        else:
            description = None
        timestamp = get_element_attribute(
            self.driver, "class:promo-timestamp", "data-timestamp", element
        )
        date = datetime.fromtimestamp(int(timestamp) / 1000)
        if element_exist(self.driver, "css:picture img", element):
            img = get_element_attribute(
                self.driver, "css:picture img", "src", element
            )
            filename = img.split("%")[-1][2:]
            picture_name = (
                f"{filename}.jpeg" if "." not in filename else filename
            )
        else:
            picture_name = None
        self.news.append(
            News(
                ...
                count=News.count_phrases(
                    [title, description], self.variables["phrase"]
                ),
                has_money=News.money_checker([title, description]),
            )
        )
    ...
    def zip_pictures(self):
        ...
        if files:
            ...
            with zipfile.ZipFile(
                f"{OUTPUT_DIR}/pictures.zip", "w", zipfile.ZIP_DEFLATED
            ) as zf:
                for file in files:
